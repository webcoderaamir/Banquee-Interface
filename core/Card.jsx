...
export const Card = ({ icon, heading, text, variant }) => {
  return (
    <AnimationContainer
      className="flex flex-col gap-4 rounded-2xl border-2 border-grey overflow-hidden p-8 "
      amount={0.1}
    >
      <BadgeL>{icon}</BadgeL>
      <div className="flex flex-col gap-2 -ml-4 self-stretch xl:w-40 2xl:w-[13.75rem]">
        <h5 className="font-heading text-xl font-medium tracking-tight">
          {heading}
        </h5>
        <p className="w-44">{text}</p>
      </div>
    </AnimationContainer>
  );
};
